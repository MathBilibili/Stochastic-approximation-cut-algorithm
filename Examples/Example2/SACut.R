...
task_id <- as.numeric(task_id_string)+5
...
denT<-function(t_n,t,P){
  re<-dnorm(as.numeric(t_n),mean = as.numeric(t),sd=2)*P 
  return(re)
}
...
init<-list(theta=1,phi=apply(PhiC,MARGIN = 2,median),t=as.matrix(1),I=1)          #t should be inversed vector.
...
MA_ex<-function(Aux_Tt,init,Z,Y,PhiC,num_run=1000,burn_in=500,thin=1){
  ...
  if(!is_Unix){
    clusterExport(cl, list('py','Y','ind_var','dnorm'), envir=environment())
  }
  foreach(i = icount(num_run)) %do%{
    if((i<=burn_in)|(i%%thin!=0)){
      for(k in 1:1){
...
q1<-Sys.time()
Result<-MA_ex(MA_aux_out,init,Z,Y,PhiC,num_run=50000,burn_in=0,thin=1)
q2<-Sys.time()
