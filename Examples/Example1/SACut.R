...
prox<-function(phi_n,phi){
  out<-sum(log(dtruncnorm(phi_n, a=0, b=100, mean = phi, sd = 0.05))) 
  return(out) 
}
...
rprox<-function(phi){
  out<-rtruncnorm(1, a=0, b=100, mean = phi, sd = 0.05)
  return(out)
}
...
tranT<-function(t){
  re<-rtruncnorm(1,a=0, b=100,mean = t,sd=0.1)  %>% t()
  return(re)
}
...
denT<-function(t_n,t,P){
  re<-dtruncnorm(t_n,a=0, b=100,mean = t,sd=0.1)*P  
  return(re)
}
...
init<-list(theta=rep(1,d_x),phi=apply(PhiC,MARGIN = 2,median),t=as.matrix(2),I=1)          #t should be inversed vector.
...
MA_ex<-function(Aux_Tt,init,Z,Y,PhiC,num_run=1000,burn_in=500,thin=1){
  ...
  if(!is_Unix){
    clusterExport(cl, list('py','Y','dmvnorm','n'), envir=environment())
  }
  foreach(i = icount(num_run)) %do%{
    if((i<=burn_in)|(i%%thin!=0)){
      for(k in 1:1){
    ...
    if(i %in% seq(1000,500000,1000)){
      Tem_out<-list(phi=sto.phi[1:((i-burn_in)/thin),10],auxphi=sto.auphi[1:((i-burn_in)/thin)],theta=sto.theta[1:((i-burn_in)/thin),],auxtheta=sto.autheta[1:((i-burn_in)/thin),],time=sto.time[1:((i-burn_in)/thin)],Tt=Tt[1,],log.Ptau_fenmu=log.numr-log.fenzi_o)
      Tem_RR<-data.table(phi= Tem_out$phi,auxphi= Tem_out$auxphi,theta= Tem_out$theta,auxtheta= Tem_out$auxtheta,time= Tem_out$time)
      write.csv(Tem_RR,paste("Liu_Result",task_id,".csv",sep = ""))
...
RR<-data.table(phi=Result$phi[,10],auphi=Result$auphi,theta=Result$theta,autheta=Result$autheta,time=Result$time)
TauPro<-data.table(Tt=Result$Tt,Ptau=Result$log.Ptau_fenmu)
write.csv(RR,paste("Liu_Result",task_id,".csv",sep = ""))
