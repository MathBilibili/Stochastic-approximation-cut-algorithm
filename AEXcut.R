...
proy<-function(theta_n,theta){
  out<-log(dtruncnorm(theta_n, a=0, b=Inf, mean = theta, sd = 0.1))
  return(out)
}

#proposal tau
prot<-function(tau_n,tau){
  out<-log(dtruncnorm(tau_n, a=0, b=Inf, mean = tau, sd = 0.1))
  return(out)
}
...
#proposal 2 samling function
rproy<-function(theta){
  out<-rtruncnorm(1, a=0, b=Inf, mean = theta, sd = 0.1)
  return(out)
}

#proposal tau samling function
rprot<-function(tau){
  out<-rtruncnorm(1, a=0, b=Inf, mean = tau, sd = 0.1)
  return(out)
}


#Choice of Auxiliary Parameter Set internal chain
MAi<-function(ini,Y,phi,num=1000){
  tau<-ini
  for(i in 1:num){
    tau_n<-rprot(tau)
    rate<-py(Y,tau_n,phi)+prot(tau,tau_n)-py(Y,tau,phi)-prot(tau_n,tau)
    alfa<-min(1,exp(rate))
    rpan<-runif(1)
    tau<-tau_n*sign(rpan<=alfa)+tau*sign(rpan>alfa)
  }
  return(tau)
}


#Choice of Auxiliary Parameter Set external chain
init<-list(theta=1,phi=rep(1,N),tau=1)
MAux<-function(init,Z,Y,num_run=1000,burn_in=500,thin=1,r){
  theta<-init$theta
  phi<-init$phi
  tau<-init$tau
  ...
  for(i in 1:num_run){
    if((i<=burn_in)|(i%%thin!=0)){
      phi_n<-rprox(phi)
      theta_n<-rproy(theta)
      tau_n<-MAi(tau,Y,phi_n,num=1000)
      rate<-py(Y,theta_n,phi_n)+px(phi_n,Z)+py(Y,tau,phi)+prox(phi,phi_n)+proy(theta,theta_n)-
        py(Y,theta,phi)-px(phi,Z)-py(Y,tau_n,phi_n)-prox(phi_n,phi)-proy(theta_n,theta)
      alfa<-min(1,(exp(rate))^r)
      rpan<-runif(1)
...
#external chain
init<-list(theta=MA_aux_out$theta,phi=MA_aux_out$phi,tau=MA_aux_out$Tt[,sample(seq(1:dim(MA_aux_out$Tt)[2]),1)],t=MA_aux_out$t,I=MA_aux_out$I)          #tau and t should be inversed vector.
